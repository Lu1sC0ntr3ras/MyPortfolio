    ...
    <div className={`${page} scroll_y`}>
      ...
      >
        ...
        <div className='content_title'>
          ...
          {open[2] ? <ArrowTop fill={color}/> : <ArrowBottom fill={color}/>}
        </div>
        <p className={`${text} information fs-14`}>{`
          To develop programs and functional applications, in my learning 
          time and beginnings as a web developer, not much is required to 
          start, also I think that only 1 of the below is a programming 
          language, and the other is just the superset of other.
        `}</p>
      ...
      <div className={`container_circle g-10 ${open[2] ? 'expanded' : ''}`}
        ...
        <p className={`${text} information fs-14`}>{`
          Work tools to develop the visual part of my applications, 
          efficiently, pleasant to the user and clean code.
        `}</p>
        <div className='circles'>
          <CircularProgressBar
            title='React'
      ...
      <div className={`container_circle g-10 ${open[3] ? 'expanded' : ''}`}
        ...
        <div className='content_title'>
          ...
          {open[3] ? <ArrowTop fill={color}/> : <ArrowBottom fill={color}/>}

        </div>
        <p className={`${text} information fs-14`}>{`
          Connect my applications with databases and allow it to be 
          saved, to be viewed at any time, for now I master these 
          programming tools.
        `}</p>
        <div className='circles'>
          <CircularProgressBar
      ...
      <div className={`container_circle g-10 ${open[4] ? 'expanded' : ''}`}
        ...
        <div className='content_title'>
          ...
          {open[4] ? <ArrowTop fill={color}/> : <ArrowBottom fill={color}/>}
        </div>
        <div className='circles'>
          <CircularProgressBar
            title='Git'
